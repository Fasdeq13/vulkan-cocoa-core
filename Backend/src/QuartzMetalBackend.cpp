...
enum class GPUVendor { Intel, NVIDIA, AMD, Apple, Unknown };
...
static void cg_store_buffer(CGContextRef ctx, void* data, size_t bpr, size_t h) {
    ...
    g_cgPixelBuffers[key].assign(static_cast<uint8_t*>(data), static_cast<uint8_t*>(data) + bpr * h);
}
...
VulkanRenderContext::VulkanRenderContext()
    : instance(VK_NULL_HANDLE), physicalDevice(VK_NULL_HANDLE), device(VK_NULL_HANDLE), graphicsQueue(VK_NULL_HANDLE), graphicsQueueFamilyIndex(0) {
}
...
void VulkanRenderContext::selectPhysicalDevice() {
    ...
    const char* vendorName = (vendor == GPUVendor::Intel) ? "Intel (UMA)" :
        ...
        (vendor == GPUVendor::AMD) ? "AMD (discrete)" :
        (vendor == GPUVendor::Apple) ? "Apple Silicon" : "Unknown";
    std::cout << "[QMB] Selected GPU: " << finalProps.deviceName << " | Vendor: " << vendorName << "\n";
}
...
void VulkanRenderContext::createLogicalDevice() {
    ...
    if (graphicsQueueFamilyIndex == UINT32_MAX) throw std::runtime_error("[QMB] No graphics queue family found");
    float priority = 1.0f;
    ...
    vk12Features.pNext = &descIndexing;
    ...
    if (vkCreateDevice(physicalDevice, &deviceCI, nullptr, &device) != VK_SUCCESS) throw std::runtime_error("[QMB] vkCreateDevice failed");
    vkGetDeviceQueue(device, graphicsQueueFamilyIndex, 0, &graphicsQueue);
}

static void createSwapchain(VkInstance instance, VkPhysicalDevice physDev, VkDevice device, uint32_t queueFamily, uint32_t width, uint32_t height) {
    VkHeadlessSurfaceCreateInfoEXT hsCI{};
    hsCI.sType = VK_STRUCTURE_TYPE_HEADLESS_SURFACE_CREATE_INFO_EXT;
    PFN_vkCreateHeadlessSurfaceEXT vkCreateHeadlessSurfaceEXT_fn = reinterpret_cast<PFN_vkCreateHeadlessSurfaceEXT>(vkGetInstanceProcAddr(instance, "vkCreateHeadlessSurfaceEXT"));
    if (!vkCreateHeadlessSurfaceEXT_fn) return;
    if (vkCreateHeadlessSurfaceEXT_fn(instance, &hsCI, nullptr, &g_swapchain.surface) != VK_SUCCESS) throw std::runtime_error("[QMB] vkCreateHeadlessSurfaceEXT failed");
    ...
    vkGetPhysicalDeviceSurfacePresentModesKHR(physDev, g_swapchain.surface, &presentModeCount, nullptr);
    std::vector<VkPresentModeKHR> presentModes(presentModeCount);
    vkGetPhysicalDeviceSurfacePresentModesKHR(physDev, g_swapchain.surface, &presentModeCount, presentModes.data());
    ...
    scCI.minImageCount = std::max(2u, surfCaps.minImageCount);
    ...
    if (vkCreateSwapchainKHR(device, &scCI, nullptr, &g_swapchain.swapchain) != VK_SUCCESS) throw std::runtime_error("[QMB] vkCreateSwapchainKHR failed");
    uint32_t imgCount = 0;
...
VkDeviceMemory VulkanRenderContext::allocateVideoMemory(VkBuffer buffer, VkMemoryPropertyFlags props) {
    VkMemoryRequirements req;
    vkGetBufferMemoryRequirements(device, buffer, &req);
    GPUVendor vendor = classifyVendor(caps.vendorID);
    if ((vendor == GPUVendor::Intel || vendor == GPUVendor::Apple) && !(props & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)) {
    ...
    VkDeviceMemory mem;
    if (vkAllocateMemory(device, &ai, nullptr, &mem) != VK_SUCCESS) throw std::runtime_error("[QMB] vkAllocateMemory (buffer) failed");
    vkBindBufferMemory(device, buffer, mem, 0);
    return mem;
}

VkDeviceMemory VulkanRenderContext::allocateImageMemory(VkImage image, VkMemoryPropertyFlags props) {
    VkMemoryRequirements req;
    ...
    ai.memoryTypeIndex = findMemoryType(physicalDevice, req.memoryTypeBits, props);
    VkDeviceMemory mem;
    if (vkAllocateMemory(device, &ai, nullptr, &mem) != VK_SUCCESS) throw std::runtime_error("[QMB] vkAllocateMemory (image) failed");
    vkBindImageMemory(device, image, mem, 0);
    return mem;
}

void VulkanRenderContext::uploadPixelsToVkImage(VkImage dstImage, VkCommandPool cmdPool, const void* srcPixels, VkDeviceSize byteSize, uint32_t w, uint32_t h) {
    GPUVendor vendor = classifyVendor(caps.vendorID);
    if (vendor == GPUVendor::Intel || vendor == GPUVendor::Apple) {
        VkBufferCreateInfo hostBufferCI{};
        hostBufferCI.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
        hostBufferCI.size = byteSize;
        hostBufferCI.usage = VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
        hostBufferCI.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
        VkBuffer hostBuffer;
        vkCreateBuffer(device, &hostBufferCI, nullptr, &hostBuffer);
        VkDeviceMemory hostMemory = allocateVideoMemory(hostBuffer, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
        void* mapped = nullptr;
        if (vkMapMemory(device, hostMemory, 0, byteSize, 0, &mapped) == VK_SUCCESS) {
        ...
        vkCmdCopyBufferToImage(cb, hostBuffer, dstImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
    ...
    VkDeviceMemory stagingMem = allocateVideoMemory(stagingBuf, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
    void* mapped;
    ...
    region.imageSubresource.layerCount = 1;
    region.imageExtent = { w, h, 1 };
    vkCmdCopyBufferToImage(cb, stagingBuf, dstImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
    barrier.oldLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
    barrier.newLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    barrier.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
    barrier.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
...
void VulkanRenderContext::initContext() {
    ...
    if (vkCreateInstance(&instCI, nullptr, &instance) != VK_SUCCESS) throw std::runtime_error("[QMB] vkCreateInstance failed");
...
static RavynOSEvent decodeMachMessage(mach_msg_header_t* hdr) {
    ...
    if (hdr->msgh_size > sizeof(mach_msg_header_t)) {
        const uint32_t* payload = reinterpret_cast<const uint32_t*>(reinterpret_cast<const uint8_t*>(hdr) + sizeof(mach_msg_header_t));
...
static void presentNextFrame(VulkanRenderContext* ctx) {
    ...
    vkWaitForFences(ctx->getVkDevice(), 1, &g_swapchain.inFlight, VK_TRUE, UINT64_MAX);
    vkResetFences(ctx->getVkDevice(), 1, &g_swapchain.inFlight);
    uint32_t imgIdx = 0;
    VkResult res = vkAcquireNextImageKHR(ctx->getVkDevice(), g_swapchain.swapchain, UINT64_MAX, g_swapchain.imageAvailable, VK_NULL_HANDLE, &imgIdx);
    ...
    vkQueuePresentKHR(ctx->getVkGraphicsQueue(), &presentInfo);
}

static void runDarwinEventLoop(VulkanRenderContext* ctx) {
    mach_port_name_t displayPort = MACH_PORT_NULL;
    kern_return_t kr = mach_port_allocate(mach_task_self(), MACH_PORT_RIGHT_RECEIVE, &displayPort);
    if (kr != KERN_SUCCESS) {
    ...
    if (displayPort != MACH_PORT_NULL) {
        ...
        EV_SET64(&ev, displayPort, EVFILT_MACHPORT, EV_ADD | EV_ENABLE, 0, 0, 0, 0, 0);
        kevent64(kq, &ev, 1, nullptr, 0, 0, nullptr);
    }
    {
        struct kevent64_s timer {};
        EV_SET64(&timer, 1, EVFILT_TIMER, EV_ADD | EV_ENABLE, NOTE_MSECONDS, 16, 0, 0, 0);
    ...
    bool running = true;
    while (running) {
        struct timespec timeout { 0, 32000000 };
        ...
        for (int i = 0; i < nev; i++) {
            ...
            if (ev.filter == EVFILT_MACHPORT && displayPort != MACH_PORT_NULL) {
                ...
                while (true) {
                    kern_return_t r = mach_msg(hdr, MACH_RCV_MSG | MACH_RCV_TIMEOUT, 0, sizeof(msgBuf), displayPort, 0, MACH_PORT_NULL);
...
static void transformAppleMemoryBarriers(llvm::Module* M) {
    ...
    constexpr uint32_t kSemanticsAcqRel = 0x8 | 0x100;
    llvm::FunctionType* barrierFT = llvm::FunctionType::get(llvm::Type::getVoidTy(ctx), { llvm::Type::getInt32Ty(ctx), llvm::Type::getInt32Ty(ctx), llvm::Type::getInt32Ty(ctx) }, false);
    llvm::FunctionCallee spirvBarrier = M->getOrInsertFunction("spirv.ControlBarrier", barrierFT);
        ...
        for (auto& BB : F) {
            for (auto& I : BB) {
                if (auto* call = llvm::dyn_cast<llvm::CallInst>(&I)) {
                    ...
                    if (name.starts_with("air.threadgroup_barrier") || name.starts_with("air.mem_barrier") || name.starts_with("air.simdgroup_barrier")) {
...
static void injectBindlessDescriptors(llvm::Module* M, llvm::NamedMDNode* stageNode, const std::string& stageName) {
    llvm::LLVMContext& ctx = M->getContext();
    llvm::NamedMDNode* spirvDecorate = M->getOrInsertNamedMetadata("spirv.Decorate");
    uint32_t descSet = (stageName == "air.vertex") ? 0 : (stageName == "air.fragment") ? 1 : 2;
    uint32_t binding = 0;
    for (unsigned i = 0; i < stageNode->getNumOperands(); i++) {
        ...
        for (unsigned j = 0; j < node->getNumOperands(); j++) {
            auto* cMD = llvm::dyn_cast<llvm::ConstantAsMetadata>(node->getOperand(j).get());
            if (!cMD) continue;
            auto* ci = llvm::dyn_cast<llvm::ConstantInt>(cMD->getValue());
            if (!ci) continue;
            uint64_t metalRegIdx = ci->getZExtValue();
            llvm::Metadata* bindArgs[] = {
                llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(llvm::Type::getInt32Ty(ctx), metalRegIdx)),
                llvm::ValueAsMetadata::get(llvm::ConstantInt::get(llvm::Type::getInt32Ty(ctx), 33)),
                llvm::ValueAsMetadata::get(llvm::ConstantInt::get(llvm::Type::getInt32Ty(ctx), binding++))
            };
            ...
            llvm::Metadata* setArgs[] = {
                ...
                llvm::ValueAsMetadata::get(llvm::ConstantInt::get(llvm::Type::getInt32Ty(ctx), descSet))
            };
            spirvDecorate->addOperand(llvm::MDNode::get(ctx, setArgs));
            llvm::Metadata* nuArgs[] = {
                llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(llvm::Type::getInt32Ty(ctx), metalRegIdx)),
                llvm::ValueAsMetadata::get(llvm::ConstantInt::get(llvm::Type::getInt32Ty(ctx), 5300))
            };
...
static void parseAppleMetalMetadata(llvm::Module* M) {
    ...
    llvm::Metadata* srcArgs[] = {
        llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(llvm::Type::getInt32Ty(ctx), 5)),
        llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(llvm::Type::getInt32Ty(ctx), 200000))
    };
    spirvSource->addOperand(llvm::MDNode::get(ctx, srcArgs));
    auto* caps = M->getOrInsertNamedMetadata("spirv.Capability");
    for (uint32_t cap : { 5, 4, 6, 4422, 4423, 4424, 4425, 4426, 4427 }) {
        llvm::Metadata* capArg[] = {
            llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(llvm::Type::getInt32Ty(ctx), cap))
        };
...
extern "C" {
    ...
    CGColorSpaceRef CGColorSpaceCreateDeviceRGB(void) {
        return reinterpret_cast<CGColorSpaceRef>(0x1);
    }
    ...
    void CGContextClearRect(CGContextRef c, CGRect rect) {
        ...
        for (int row = y0; row < y1; row++) {
            uint32_t* p = reinterpret_cast<uint32_t*>(desc->pixels.data() + row * desc->bytesPerRow);
    ...
    int NSApplicationMain(int argc, const char* argv[]) {
        try {
            ...
            id appInstance = ((id(*)(id, SEL))objc_msgSend)((id)appClass, sel_registerName("sharedApplication"));
            if (appInstance) {
...
static id sampleNextDrawableImplementation(id self, SEL _cmd) {
    ...
    id inst = ((id(*)(id, SEL))objc_msgSend)((id)drawableClass, sel_registerName("alloc"));
    ...
    if (bridgeIvar) {
        ...
        bridge->vkImage = (g_swapchain.images.empty()) ? VK_NULL_HANDLE : g_swapchain.images[g_swapchain.currentImageIdx];
...
static void drawablePresent_impl(id self, SEL _cmd) {
    ...
    auto* bridge = reinterpret_cast<QuartzMetalTextureBridge*>(object_getIvar(self, bridgeIvar));
    ...
    if (g_swapchain.renderFinished != VK_NULL_HANDLE) {
        ...
        vkQueueSubmit(ctx->getVkGraphicsQueue(), 1, &si, VK_NULL_HANDLE);
    }
    delete bridge;
...
static id winInit_func(id self, SEL _cmd, CGRect rect, unsigned long mask, unsigned long backing, bool defer) {
    return self;
}
...
void VulkanRenderContext::buildDynamicObjectiveCBridge() {
    ...
    if (!objc_getClass("CAMetalLayer")) {
        ...
        if (c) {
            class_addMethod(c, sel_registerName("nextDrawable"), (IMP)sampleNextDrawableImplementation, "@@:");
    ...
    if (!objc_getClass("QuartzMetalDrawable")) {
        ...
        if (c) {
            class_addIvar(c, "vulkanTextureBridge", sizeof(void*), static_cast<uint8_t>(std::log2(sizeof(void*))), "^v");
            class_addIvar(c, "parentLayer", sizeof(id), static_cast<uint8_t>(std::log2(sizeof(id))), "@");
